...
namespace DirectNXAML.DrawData
{
    ...
    public class FVertex3D : Primitive
    {
        ...
        public FVertex3D(Vector3 _pos, Vector3 _nor) : this(_pos)
        {
            m_lp.nor = _nor;
        }
        public FVertex3D(Vector3 _pos, Vector4 _col) : this(_pos)
        {
            m_lp.col = _col;
        }
        public FVertex3D(Vector3 _pos, Vector3 _nor, Vector2 _tex) : this(_pos, _nor)
        {
            m_lp.tex = _tex;
        }
        public FVertex3D(Vector3 _pos, Vector3 _nor, Vector2 _tex, Vector4 _col) : this(_pos, _nor, _tex)
        {
            m_lp.col = _col;
        }
